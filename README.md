...
Notebooks 06 and 07 plus `genie_seed_questions.md` are optional extensions that build on the agent-ready surface produced by notebook 05. They do not change the core 01–05 pipeline:

- `06_Configure_Genie_Space.py` — programmatic setup of a Databricks Genie Space grounded in the schema and UC functions from 01–05. Falls back to manual UI steps if the Genie management API is not yet available in the workspace.
- `07_Bedrock_AgentCore_Integration.md` — reference architecture for calling the UC SQL functions from an Amazon Bedrock AgentCore agent, including OAuth M2M setup and a Lambda action-group code stub.
- `genie_seed_questions.md` — curated example questions for the Genie Space, each mapped to the UC SQL function it should invoke.
